...
export const getChatCustomers = asyncHandler(async (req, res) => {
  ...
  const customerFilter = {
    ...
    // but drop out of the list. Written as an exclusion, not `=== ACTIVE`:
    // accountStatus was added after customers already existed, so the field is
    // simply absent on older documents and an equality match dropped every one
    // of them — which emptied this column entirely. $nin matches a missing
    // field, equality does not. See ACCOUNT_VISIBLE.
